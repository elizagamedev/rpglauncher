...
char *(*_libguess_determine_encoding)(const char *inbuf,
                                      int length,
...
static int getEncoding(const char *str)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(langs); ++i) {
        if ((encStr = _libguess_determine_encoding(str, length, regions[i]))
                != NULL)
            break;
    }
    if (encStr == NULL)
...
BOOL codepage_init(const Path filename)
{
    if (!(libguess = LoadLibraryW(filename)))
        return FALSE;
    if (!(_libguess_determine_encoding
            = (void*)GetProcAddress(libguess, "libguess_determine_encoding")))
        return FALSE;
    return TRUE;
}
